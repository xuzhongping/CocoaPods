    ...
    class Analyzer
      ...
      def analyze(allow_fetches = true)
        ...
        if installation_options.integrate_targets?
          # 预存Podfile 中每个target生成所有的检查结果 [Hash<TargetDefinition => TargetInspectionResult>]
        ...
        # 根据podfile和podfile.lock文件来判断pod要进行的行为，返回代表行为的状态
        podfile_state = generate_podfile_state
        # 保存external pod的 checkout options
      ...
      def fetch_external_sources(podfile_state)
        ...
        # 根据podfile_state生成的需要更新的pods
        pods = pods_to_fetch(podfile_state)
        return if deps.empty?
        UI.section 'Fetching external sources' do
          deps.sort.each do |dependency|
            # !pods.include?(dependency.root_name) 这个表达式的返回值，指的是：
            # 由于需要更新的external pods，并不一定全部是因为podfile_state中的原因导致更新，比如可能是因为缓存不存在，
            # 所以如果是因为podfile_state中的原因导致的更新，就不再使用lock中的options，否则依然使用lock中的options。
